...
def plot_metrics_distribution(fold_metrics):
    ...
    plt.tight_layout()
    plt.savefig('Results/DSM/Graphs/metrics_distribution.png')
    plt.close()
...
def analyze_confusion_matrix(true_labels, predictions):
    ...
    plt.tight_layout()
    plt.savefig('Results/DSM/Graphs/confusion_matrix.png')
    plt.close()
...
def analyze_error_cases(texts, predictions, true_labels):
    ...
    error_df.to_csv('Results/DSM/CSV_files/error_analysis.csv', index=False)
...
def analyze_feature_importance():
    ...
    results_dir = Path('Results/DSM/Fold_analysis')
    ...
    plt.tight_layout()
    plt.savefig('Results/DSM/Graphs/feature_importance.png')
    plt.close()
...
def analyze_fold_performance(fold_metrics):
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('Results/DSM/Graphs/fold_performance.png')
    plt.close()
...
def generate_text_length_analysis(texts, predictions, true_labels):
    ...
    plt.tight_layout()
    plt.savefig('Results/DSM/Graphs/text_length_analysis.png')
    plt.close()
...
def generate_full_report(results_path):
    ...
    # Save summary report
    with open('Results/DSM/JSON_files/analysis_summary.json', 'w') as f:
...
def main():
    ...
    results_dir = Path('Results/DSM/JSON_files')
