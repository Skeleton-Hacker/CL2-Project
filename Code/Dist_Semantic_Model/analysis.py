...
def plot_metrics_distribution(fold_metrics):
    ...
    plt.tight_layout()
    plt.savefig('Results/Graphs/metrics_distribution.png')
    plt.close()
...
def analyze_error_cases(texts, predictions, true_labels):
    ...
    error_df.to_csv('Results/CSV_files/error_analysis.csv', index=False)
...
def analyze_feature_importance():
    ...
    results_dir = Path('Results/Fold_analysis')
    ...
    plt.tight_layout()
    plt.savefig('Results/Graphs/feature_importance.png')
    plt.close()
...
def analyze_fold_performance(fold_metrics):
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('Results/Graphs/fold_performance.png')
    plt.close()
...
def generate_text_length_analysis(texts, predictions, true_labels):
    ...
    plt.tight_layout()
    plt.savefig('Results/Graphs/text_length_analysis.png')
    plt.close()
...
def generate_full_report(results_path):
    ...
    # Save summary report
    with open('Results/JSON_files/analysis_summary.json', 'w') as f:
...
def main():
    ...
    results_dir = Path('Results/JSON_files')
